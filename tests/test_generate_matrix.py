...
class TestGetDefaultCombinations:
    ...
    @pytest.mark.parametrize(
        "event_name",
        ["push", "pull_request", "issue_comment", "workflow_dispatch"],
    )
...
class TestMain:
    ...
    def _run_main(
        ...
        *,
        ...
        """Run main() with mocked env vars and return the matrix."""
        ...
        matrix_line = next(line for line in content.splitlines() if line.startswith("matrix="))
        return json.loads(matrix_line.split("=", 1)[1])["include"]

    def test_no_input_pr_event(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
    ...
    def test_explicit_single_combo(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
        ...
        result = self._run_main(
            monkeypatch, tmp_path, event="pull_request", user_input="clang/asan"
        )
        assert result == [{"compiler": "clang", "sanitizer": "asan"}]

    def test_explicit_multiple_combos(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
        ...
        result = self._run_main(
            ...
            tmp_path,
            event="pull_request",
            user_input="gcc/none clang/tsan",
        )
        ...
        assert sorted(
            ...
            [
                {"compiler": "gcc", "sanitizer": "none"},
                {"compiler": "clang", "sanitizer": "tsan"},
            ],
            key=lambda x: (x["compiler"], x["sanitizer"]),
        )
    ...
    def test_plus_modifier(
        ...
        result = self._run_main(
            monkeypatch, tmp_path, event="pull_request", user_input="+clang/none"
        )
        ...
        assert sorted(
            ...
            [
                {"compiler": "gcc", "sanitizer": "none"},
                {"compiler": "clang", "sanitizer": "none"},
            ],
            key=lambda x: (x["compiler"], x["sanitizer"]),
        )

    def test_minus_modifier(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
        ...
        result = self._run_main(
            ...
            tmp_path,
            event="pull_request",
            user_input="-gcc/none +clang/none",
        )
        assert result == [{"compiler": "clang", "sanitizer": "none"}]

    def test_issue_comment_with_build_command(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
    ...
    def test_issue_comment_no_match(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
